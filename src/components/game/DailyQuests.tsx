...
export function DailyQuests() {
  ...
  return <motion.div initial={{
    ...
    opacity: 1,
    y: 0
  }} className="rounded-2xl overflow-hidden relative" style={{
    ...
    boxShadow: '0 0 30px rgba(56, 189, 248, 0.3), inset 0 0 20px rgba(250, 204, 21, 0.2)'
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {[...Array(8)].map((_, i) => <motion.div key={i} className="absolute w-2 h-2 rounded-full" style={{
      ...
      {/* Header */}
      <motion.div className="px-4 py-4 flex items-center justify-between relative z-10" style={{
      ...
      borderBottom: '3px solid #cc3700'
        ...
        <div className="absolute inset-0 overflow-hidden">
          {[...Array(5)].map((_, i) => <motion.div key={i} className="absolute bottom-0" style={{
          left: `${i * 25}%`,
          width: '30px',
          height: '20px',
        ...
        <div className="flex items-center gap-2 relative z-10">
          ...
          <span className="font-display font-bold text-xl text-white" style={{
          textShadow: '2px 2px 4px rgba(0,0,0,0.5), 0 0 10px rgba(255,150,0,0.5)'
        }}>Quest Giornaliere</span>
        </div>
        <motion.div className="flex items-center gap-3 text-sm" animate={{
        scale: [1, 1.05, 1]
      }} transition={{
        duration: 1.5,
        repeat: Infinity
      }}>
          <span className="px-3 py-1 rounded-full text-white font-bold" style={{
      ...
      {/* Summary */}
      {completedCount > 0 && <motion.div className="px-4 py-3 flex items-center justify-between relative z-10" initial={{
      opacity: 0
    }} animate={{
      opacity: 1
    }} style={{
      ...
      borderBottom: '2px solid rgba(255,150,0,0.3)'
          ...
          <div className="flex items-center gap-4 text-sm">
            <motion.span className="flex items-center gap-1 text-yellow-400 font-bold" animate={{
      ...
      <div className="p-3 space-y-2 max-h-[400px] overflow-y-auto relative z-10">
        ...
        return <motion.div key={quest.id} initial={{
          opacity: 0,
          x: -20
        }} animate={{
          opacity: 1,
          ...
          scale: 1.02,
          x: 5
        }} className="rounded-xl p-3 transition-all relative overflow-hidden" style={{
          background: isCompleted ? 'linear-gradient(135deg, rgba(34,197,94,0.3) 0%, rgba(22,163,74,0.2) 100%)' : isHot ? 'linear-gradient(135deg, rgba(255,69,0,0.2) 0%, rgba(255,100,0,0.1) 100%)' : 'linear-gradient(135deg, rgba(255,255,255,0.08) 0%, rgba(255,255,255,0.03) 100%)',
          border: isCompleted ? '2px solid rgba(34,197,94,0.5)' : isHot ? '2px solid rgba(255,100,0,0.4)' : '1px solid rgba(255,255,255,0.1)',
          boxShadow: isHot && !isCompleted ? '0 0 15px rgba(255,69,0,0.2)' : 'none'
        }}>
              {/* Fire effect for hot quests */}
              {isHot && !isCompleted && <motion.div className="absolute right-2 top-1/2 -translate-y-1/2" animate={{
            scale: [1, 1.2, 1],
            rotate: [-5, 5, -5]
          }} transition={{
            duration: 0.5,
            repeat: Infinity
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                duration: 0.5
                    ...
                    </motion.div> : <Target className={`w-5 h-5 ${isHot ? 'text-orange-400' : 'text-muted-foreground'}`} />}
                  <span className={`font-bold ${isCompleted ? 'text-green-400' : isHot ? 'text-orange-300' : 'text-foreground'}`}>
                    Quest {quest.id}
                  </span>
                ...
                <div className="flex items-center gap-3 text-xs">
                  ...
                  <motion.span className="flex items-center gap-1 text-yellow-400 font-bold" animate={isHot && !isCompleted ? {
                scale: [1, 1.1, 1]
              } : {}} transition={{
                duration: 0.5,
                repeat: Infinity
              ...
              {!isCompleted && <div className="space-y-1">
              ...
              border: '1px solid rgba(255,255,255,0.1)'
                ...
                width: `${progress}%`,
                background: isHot ? 'linear-gradient(90deg, #ff4500 0%, #ff8c00 50%, #ffcc00 100%)' : 'linear-gradient(90deg, #3b82f6 0%, #60a5fa 100%)',
              ...
              {isCompleted && <motion.div className="text-xs text-green-400 font-medium flex items-center gap-1" initial={{
            opacity: 0,
            y: 5
          }} animate={{
            opacity: 1,
      ...
      {/* Footer */}
      <motion.div className="px-4 py-3 text-center text-xs relative z-10" style={{
